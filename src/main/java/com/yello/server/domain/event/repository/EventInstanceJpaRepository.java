...
public interface EventInstanceJpaRepository extends JpaRepository<EventInstance, Long> {
    ...
    @Query("select e from EventInstance e, EventHistory eh where e.eventHistory = eh and e.event = ?1 and eh.user = ?2")
    List<EventInstance> findAllByEventAndUser(Event event, User user);
}
